...
export function buildSupervisorPrompt({
  ...
  if (immersiveDomains.length > 0) {
    lines.push(
      `7. Если пользователь говорит "погружаемся в X" / "давай в X" / "открой режим X" / "зайди в X" / "давай займёмся X" / "займёмся X" / "займусь X" (X — название домена или его очевидный описательный аналог, например "списком покупок" → shopping) и совпадает с одним из immersive-доменов (${immersiveDomains.join(", ")}) — выбирай goto="__immersive_open__", task="<имя домена из списка>". НЕ роути в обычный домен в этом случае.`,
    );
  }
  return lines.join("\n");
}
