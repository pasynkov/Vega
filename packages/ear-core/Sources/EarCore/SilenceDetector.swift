...
public final class SilenceDetector {
    public struct Config {
        public var endSilenceMs: Int = 3_000
        public var graceMs: Int = 500
        public var calibrationMs: Int = 600
        public var speechMargin: Double = 300
        public var silenceMargin: Double = 100
        public var fallbackNoiseFloor: Double = 100
        public init() {}
    }
    ...
    public func setEndpointSuppressed(_ suppressed: Bool) {
        if suppressEndpoint != suppressed {
            NSLog("[EarCore] VAD endpoint suppression \(suppressed ? "ON" : "OFF")")
        }
    ...
    public func feed(pcm: Data) -> Decision {
            ...
            if let started = silenceStartedAt,
               Date().timeIntervalSince(started) * 1000 >= Double(config.endSilenceMs) {
                if suppressEndpoint {
                    ...
                    NSLog("[EarCore] VAD endpoint: sustained silence \(config.endSilenceMs) ms")
                    return .endpoint
                }
            }
            logState("silence")
