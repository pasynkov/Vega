...
public final class SessionCoordinator {
    ...
    private let socket: EarSocketing
    private let cues: CuePlaying
    private let status: StatusControlling
    ...
    public var onSessionStateChange: ((Bool) -> Void)?
    ...
    public init(
        ...
        socket: EarSocketing,
        cues: CuePlaying,
        statusController: StatusControlling,
    ...
    // Test helper: blocks until any queued serial work has been processed.
    // Used by SessionCoordinatorTests after triggering an async path.
        ...
        audio.addSink { [weak self] pcm in
            ...
            self.serial.async {
                ...
                if self.sinkCallbackCount <= 3 {
                    NSLog("[EarCore] SessionCoordinator sink #\(self.sinkCallbackCount): bytes=\(pcm.count) paused=\(self.paused) activeSession=\(self.activeSessionId ?? "nil")")
                }
                if !self.paused {
    ...
    public func shutdown() {
        serial.sync {
            if let sid = activeSessionId {
                NSLog("[EarCore] Ear ending session=\(sid) initiator=ear:quit reason=user bytesSent=\(bytesSentInSession)")
    ...
    public func setPaused(_ paused: Bool) {
        serial.async {
            ...
            if paused, let sid = self.activeSessionId {
                NSLog("[EarCore] Ear ending session=\(sid) initiator=ear:pause reason=user bytesSent=\(self.bytesSentInSession)")
    ...
    public func simulateWake() {
        NSLog("[EarCore] simulateWake called")
    ...
    public func stopActiveSession() {
        serial.async {
            guard let sid = self.activeSessionId else {
                NSLog("[EarCore] stopActiveSession called but no active session")
                return
            }
            NSLog("[EarCore] Ear ending session=\(sid) initiator=ear:menu reason=user bytesSent=\(self.bytesSentInSession)")
    ...
    private func handleWake(score: Float) {
        serial.async {
            ...
            NSLog("[EarCore] Wake detected (score=\(score)), session=\(sessionId)")
    ...
    private func handleArmCapture(mode: SessionMode, captureMs: Int?) {
        serial.async {
            ...
            guard self.activeSessionId == nil else {
                NSLog("[EarCore] arm_capture ignored: session already active")
                return
            }
            let sessionId = UUID().uuidString.lowercased()
            self.activeSessionId = sessionId
            ...
            NSLog("[EarCore] arm_capture: starting session=\(sessionId) mode=\(mode.rawValue) captureMs=\(self.askCaptureMs)")
    ...
    private func streamAudio(_ pcm: Data) {
        ...
        do {
            ...
            NSLog("[EarCore] encoder error: \(error)")
        }
        ...
        if let detector = silenceDetector {
            ...
            case .endpoint:
                NSLog("[EarCore] Ear ending session=\(sid) initiator=ear:local_vad reason=vad bytesSent=\(bytesSentInSession)")
    ...
    private func endSessionLocally(sessionId: String, reason: EarEndReason) {
        ...
        NSLog("[EarCore] endSessionLocally session=\(sessionId) reason=\(reason.rawValue) bytesSent=\(bytesSentInSession)")
    ...
    private func accumulateAndMaybeReportRms(pcm: Data) {
        ...
        if now.timeIntervalSince(rmsLastReportAt) >= 1.0, !rmsAccumulator.isEmpty {
            ...
            NSLog(String(
                format: "[EarCore] mic RMS=%.0f (%.1f dBFS) samples=%d bytesSent=%d",
    ...
    private func armSafetyTimer(for sessionId: String) {
        ...
        timer.setEventHandler { [weak self] in
            ...
            NSLog("[EarCore] Ear ending session=\(sessionId) initiator=ear:safety_timer reason=timeout bytesSent=\(self.bytesSentInSession) mode=\(self.sessionMode.rawValue) cap=\(capMs)")
    ...
    private func handleAck(_ message: AckMessage) {
        NSLog("[EarCore] register acked for deviceId=\(message.deviceId)")
    }
    ...
    private func handlePartial(_ message: PartialTranscriptMessage) {
        NSLog("[EarCore] partial: \(message.text)")
    ...
    private func handleFinal(_ message: FinalTranscriptMessage) {
        NSLog("[EarCore] final: \(message.text)")
        overlay.setLiveCaption(message.text)
        serial.async { self.bumpSafetyOnTranscript(sessionId: message.sessionId) }
    }

    private func handleSessionEnd(_ message: CoreSessionEndMessage) {
        serial.async {
            NSLog("[EarCore] Core ended session=\(message.sessionId) reason=\(message.reason.rawValue) detail=\(message.detail ?? "-") bytesSent=\(self.bytesSentInSession)")
            guard self.activeSessionId == message.sessionId else { return }
            self.activeSessionId = nil
            self.safetyTimer?.cancel()
            self.safetyTimer = nil
            switch message.reason {
            case .endpoint, .timeout, .user:
                // `user` here covers Core-initiated session ends triggered
                // by the supervisor (e.g. an arm_capture bridging from a
                // regular session into immersive). Not an error — back to idle.
                self.status.setState(.idle)
            case .sttError:
                self.status.setState(.error(message.detail ?? "Session ended: stt_error"))
            }
            self.onSessionStateChange?(false)
    ...
    private func applyListViewUpdate(_ message: ListViewUpdateMessage) {
        overlay.applyListViewUpdate(message)
    }
    ...
    private func applyOverlayUpdate(_ message: OverlayUpdateMessage) {
        ...
        overlay.applyOverlayUpdate(message)
    }
    ...
    private func applySessionMode(_ msg: SessionModeChangeMessage) {
        guard activeSessionId == msg.sessionId else {
            NSLog("[EarCore] session_mode for inactive session=\(msg.sessionId), ignoring")
            return
        }
        sessionMode = msg.mode
        NSLog("[EarCore] session_mode applied: \(msg.mode.rawValue)")
