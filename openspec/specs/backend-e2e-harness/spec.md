# backend-e2e-harness Specification

## Purpose

A deterministic wire-level end-to-end test harness for `apps/core` that exercises the Ear↔Core protocol against a real per-test Nest app while keeping STT and LLM mocked. The harness gives scenario authors a tiny, stable surface (`bootTestApp`, `FakeEar`, `FakeDeepgram`, `ScriptedLlm`) so behavior is observed through the production wire — not through Nest, LangGraph, or service internals — and so the suite runs offline with zero outbound network.
