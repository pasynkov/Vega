# immersive-mode Specification

## Purpose

Fourth Ear-session mode parallel to `regular`, `continuous`, and `ask`. Lets the user "погрузиться" in a domain (e.g. shopping) and dictate a series of commands that go DIRECTLY to that domain's session-spec, bypassing the kernel orchestration graph. The session lives until a voice close-command or a 15-second silence cap; the cap is paused while the domain agent is mid-invoke so a slow LLM turn doesn't kick the user out. This capability covers the `SessionMode "immersive"` semantics, the `ImmersiveDomainRegistry` Nest-service, the `SessionAgentRunner` per-final-turn strategy, the kernel-tool `open_immersive_session`, and the wake-word suppression policy for active immersive sessions.
...
Top-supervisor SHALL be able to open an immersive session via a kernel-provided action `open_immersive_session({domain, intent?})`. The action SHALL resolve `registry.get(domain)`; if found, SHALL call `EarSessionRouter.arm({ownerSpec: reg.sessionSpec, mode: "immersive", intent, domainName: domain})`. The action SHALL NOT be exposed to any domain-supervisor-spec. Domain SHALL be validated at runtime against the live registry list.

The action MAY be implemented as either a true LangChain tool (`buildOpenImmersiveSessionTool` factory exists for that use) or as a pseudo-goto on the supervisor's `route` tool (`goto: "__immersive_open__"`, `task: "<domain>"`), provided the validation, arm semantics, and registry lookup are equivalent.

#### Scenario: Top-supervisor opens immersive session for shopping

- **WHEN** the top-supervisor receives the final `"давай займёмся списком покупок"`
- **THEN** the supervisor SHALL invoke the immersive-open action with `domain = "shopping"`
- **AND** `router.arm` SHALL be invoked with `ownerSpec = shopping-session-spec`, `mode = "immersive"`, and `domainName = "shopping"`
- **AND** the arm SHALL succeed with `{ok: true, deviceId, mode: "immersive"}`
