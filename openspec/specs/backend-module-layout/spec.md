# backend-module-layout Specification

## Purpose

Defines the four-category source layout for `apps/core/src/` (`integrations/`, `conversation/`, `tools/`, `domains/`), the rules for placing a NestJS module under each category, and the domain-registration contract (what a domain module is allowed to import and what it must do in `OnModuleInit`). Lives next to `vega-core` because the layout is binding on every Core change.
...
The test SHALL assert that `AgentRegistry.list()` contains the `notes` `AgentSpec` and does NOT contain a `memory` / `memory_search` spec (because memory is a tool, not a routable domain).

#### Scenario: Boot-time registry contract

- **WHEN** the contract e2e test is executed against the current source tree
- **THEN** the test SHALL pass
- **AND** `AgentRegistry.list()` SHALL contain the `notes` `AgentSpec`
- **AND** `AgentRegistry.list()` SHALL NOT contain a `memory` or `memory_search` `AgentSpec`
- **AND** `FlushHookRegistry.get("notes-session")` SHALL return a non-null hook
