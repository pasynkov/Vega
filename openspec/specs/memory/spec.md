...
A `memory` `AgentSpec` SHALL be registered against `AGENT_SPEC`. Its tools SHALL be exactly `search`, `write`, `update`, `delete`, each fronted by a `class-validator` DTO. Its system prompt SHALL instruct the agent to:
