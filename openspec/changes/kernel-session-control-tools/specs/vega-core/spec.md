...
The `notes` domain SHALL expose an `open_continuous_session` tool that, instead of mutating the active regular session, goes through the kernel-provided `buildOpenContinuousSessionTool` factory (in `apps/core/src/conversation/kernel/tools/`) which calls `EarSessionRouter.arm({ ownerSpec: notesAgentSpec, mode: "continuous" })`. The arm call dispatches `arm_capture` to the connected Ear so a fresh continuous session is opened and bound to the notes sub-agent. The original short utterance terminates normally via its own VAD / silence cap.
...
#### Scenario: continuous session uses the relaxed cap from start

- **WHEN** the Ear sends `session_start` with `mode: "continuous"`
- **THEN** Core SHALL set the session's silence cap to ~60 seconds (the `CONTINUOUS_MODE_SILENCE_CAP_MS` constant)
- **AND** Core SHALL suppress the per-session VAD-driven `core:vad` termination for the lifetime of the session

#### Scenario: continuous end via owning sub-agent

- **WHEN** the notes sub-agent calls `finalize_note(cleanText)` on an active continuous session
