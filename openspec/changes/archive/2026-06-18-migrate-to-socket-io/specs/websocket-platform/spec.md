...
- A per-connection `id` generator backed by `ulid` (or equivalent) so socket ids are stable for log correlation across reconnect.
...
The gateway SHALL declare `@SubscribeMessage` handlers for the following Ear → Core events; each handler SHALL validate its payload through a class-validator DTO and SHALL log + reject (without disconnecting) any payload that fails validation:
...
Core SHALL emit the following events to a single Ear socket using `socket.emit(event, payload)` (no `io.to(room)` in this change — rooms come later):
...
Each outbound event SHALL carry the exact same payload shape it carried as a `ws` text frame today; the change is the transport, not the semantics.
...
`EarRegistry` SHALL track per-device socket.io `Socket` references (not raw `ws.WebSocket`). The registry SHALL replace any prior socket for the same `deviceId` and SHALL disconnect the superseded socket with a "superseded" reason. Disconnect on either side SHALL remove the entry. Outbound paths SHALL call into a small `emitTo(deviceId, event, payload)` helper exposed by the gateway / registry so service code never reaches for the socket directly.
...
- **THEN** the prior socket SHALL be disconnected with a "superseded" reason
- **AND** the new socket SHALL replace it in the registry

### Requirement: Validation and exception handling pipeline

Inbound `@SubscribeMessage` handlers SHALL validate payloads through `ValidationPipe` + class-validator DTOs. The gateway SHALL apply an exception filter that converts thrown errors into a structured `exception` event back to the client and (when configured) into log entries; it SHALL NOT crash the connection. The filter and pipe pattern SHALL follow the structure used by `balancy-backend/apps/edge/gateway` (`AlwaysAckInterceptor` analogue is OUT of scope for this change — added later only if real ack semantics are needed).

#### Scenario: A handler throws

- **WHEN** an inbound handler throws a generic `Error`
- **THEN** the exception filter SHALL convert it to a `WsException` shape
- **AND** SHALL emit an `exception` event back to the originating client
- **AND** SHALL NOT disconnect the socket
