...
- [ ] 9.1 Create a Swift macOS app project at `apps/mac-ear/Vega Ear.xcodeproj` targeting the latest stable macOS version that supports the developer's machine. App category: Utilities. No Dock icon (`LSUIElement = YES`). _Deferred: scaffold uses SPM `Package.swift` with an executable target. Open in Xcode to materialize a project; `Info.plist` and `LSUIElement = YES` are provided at `Sources/VegaEar/Resources/Info.plist` for copy-into-target._
...
- [ ] 17.2 Verify `audio.ogg` is accepted by Telegram by manually uploading one captured file via `@BotFather`-issued test bot using `sendVoice` and confirming playback. _Ready for manual run. Core already emits real OGG/OPUS via ffmpeg-static; the unit test asserts the "OggS" magic bytes. Awaiting a live capture by the user with `PICOVOICE_ACCESS_KEY`, `DEEPGRAM_API_KEY`, and `Vega.ppn` provisioned._
- [ ] 17.3 Verify the false-positive rate by leaving the Ear idle for 30 minutes of normal background conversation and confirming no spurious sessions in `recordings/`. _Ready for manual run. Same prerequisites as 17.2._
