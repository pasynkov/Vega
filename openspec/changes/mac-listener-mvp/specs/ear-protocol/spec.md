...
All control messages (anything other than `audio_frame`) SHALL be sent as UTF-8 JSON text frames. `audio_frame` SHALL be sent as binary frames whose payload is a single OPUS-encoded packet preceded by an 8-byte little-endian unsigned integer header containing the `sessionId` short identifier as defined by the protocol package.
